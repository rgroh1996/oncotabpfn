...
Alternatively, use `uv venv --python 3.12` and `uv pip install -r requirements.txt`. `requirements.lock` records the exact tested environment. On macOS, LightGBM may require `brew install libomp`; the verified machine already had OpenMP installed.
